...
public class OH58D implements Aircraft {
    ...
    public JSONArray getCommands(List<Point> dcsPoints) {
        ...
        3091 - Clear
         */
        commandArray.put(deviceCodeDelay("3009", "11")); // HSD
        commandArray.put(deviceCodeDelay("3014", "11")); // R2
        commandArray.put(deviceCodeDelay("3004", "11")); // L4
        for (Point coord : coords) {
            commandArray.put(deviceCodeDelay("3002", "11")); // L2
            commandArray.put(deviceCodeDelay("3091", "14")); // Clear
            // Lat
            if (coord.latitudeHemisphere() == Hemisphere.NORTH) {
                commandArray.put(deviceCodeDelay("3076", "14"));
            } else {
                commandArray.put(deviceCodeDelay("3081", "14"));
            }
            enterDigits(coord.latitude(), commandArray);
            commandArray.put(deviceCodeDelay("3089", "14")); // Enter

            commandArray.put(deviceCodeDelay("3003", "11")); // L3
            commandArray.put(deviceCodeDelay("3091", "14")); // Clear
            // Long
            if (coord.longitudeHemisphere() == Hemisphere.EAST) {
                commandArray.put(deviceCodeDelay("3067", "14"));
            } else {
                commandArray.put(deviceCodeDelay("3085", "14"));
            }
            ...
            commandArray.put(deviceCodeDelay("3017", "11")); // R5 - Store
        }
        commandArray.put(deviceCodeDelay("3008", "11")); // Exit nav setup
        return commandArray;
    }

    private static void enterDigits(String coords, JSONArray commandArray) {
        for (char digit : coords.toCharArray()) {
            commandArray.put(deviceCodeDelay(tenKeys[Character.getNumericValue(digit)], "14"));
        }
    }

    private static JSONObject deviceCodeDelay(String code, String device) {
